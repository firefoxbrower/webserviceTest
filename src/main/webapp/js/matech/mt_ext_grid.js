...
var DEFAULT_GRID_URL =TEMP_SYSTEM_WEB_ROOT+ "/common.do?method=getGridJsonData";
...
Ext.matech.grid.GridPanel = Ext.extend(Ext.grid.GridPanel,{
	...
	initComponent : function() {
		...
		if(!self.tableId||self.tableId==""){
			self.tableId="list"+new UUID().createUUID();
		}
		self.id="gridId_"+self.tableId;
			...
			if(self.groupHead){
		 	...
		 	if(self.isShowBbar){
				...
				if(self.isShowClearState){
					_bbar_items=_bbar_items.concat(['-',{
													text:'清除操作习惯',
													icon:TEMP_SYSTEM_WEB_ROOT+'/img/clear.gif',
													handler:function(){
				...
				if(self.isShowCalculator){
					_bbar_items=_bbar_items.concat(['-',{
														text:'计算器',
														icon:TEMP_SYSTEM_WEB_ROOT+'/img/cal.gif',
														handler:function(){
