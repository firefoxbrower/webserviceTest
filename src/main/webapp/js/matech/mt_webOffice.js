...
Ext.matech.webOffice = function (config){
	...
	return {
		...
		init:function(){
			...
			if(!this.OpenUrl){
				this.OpenUrl="/common.do?method=attachDownload";
			}
			if(!this.SaveUrl){
				this.SaveUrl="/common.do?method=attachUpload";
			}	
			...
			AuditReport.pUserName=this.UserName;
			AuditReport.pWebRoot=TEMP_SYSTEM_WEB_ROOT;

			return AuditReport;
		},
		OpenFileDefault:function(attachId,type){
			
			this.setOpenUrl("/common.do?method=attachDownload");
			this.setSaveUrl("/common.do?method=attachUpload");
			
			if(!this.Handler){
				this.Handler="CommonHandler";
			}
			
			var url = TEMP_SYSTEM_WEB_ROOT+"/common.do?method=getAttachList";
		...
		OpenUrlFile:function(){
		    ...
		    AuditReport.pOpenUrl = matech.getlocationhost() + TEMP_SYSTEM_WEB_ROOT+this.OpenUrl+"&handler="+this.Handler+"&"+this.OpenUrlParameter;
		    AuditReport.pSaveUrl=matech.getlocationhost() + TEMP_SYSTEM_WEB_ROOT+this.SaveUrl+"&OpenMode="+this.OpenMode+"&handler="+this.Handler+"&userId="+this.UserId+"&"+this.SaveUrlParameter; 
			AuditReport.pUrlParameter=matech.getlocationhost()+"|root="+TEMP_SYSTEM_WEB_ROOT+"|&readonly="+this.isReadOnly+"&handler="+this.Handler+ this.UrlParameter;
