...
Ext.matech.form.Validation = function (config){
	...
	var rules = [
		...
		['validate-alphanum', '请输入正确的内容(字母或者数字)', function(v) {
	...
	return {
		...
		check : function (elm){
			...
			/*
			var valuemustexist = elm.valuemustexist ;
			
			if(!valuemustexist) {
				//可能是新下拉，从ext对象中找valuemustexist
				var inputId = elm.inputId ;
				if(inputId) {
					var selectCmp = Ext.getCmp(inputId) ;
					if(selectCmp)
						valuemustexist = selectCmp.valuemustexist ;
				}
			}
			
			if(isPass && !isSelectValidtate && valuemustexist && valuemustexist == "true") {
		...
		selectValidate : function(elm) {
			...
			if($.ajax){
				$.ajax({
					type :"Post",
					async:false,
					url : MATECH_SYSTEM_WEB_ROOT+"/system.do?method=combox&checkmode=1&autoid="+autoid,
